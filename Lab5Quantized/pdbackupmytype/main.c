...
int save(LeNet5 *lenet, char filename[])
{
	...
	if (!fp) return 1;
	int write_amount = fwrite(lenet, sizeof(LeNet5), 1, fp);
	printf("%d elements written, sizeofLeNet5: %lu\n", write_amount, sizeof(LeNet5));
	fclose(fp);
	return 0;
}
...
int load(LeNet5 *lenet, char filename[])
{
	...
	if (!fp) return 1;
	int read_amount = fread(lenet, sizeof(LeNet5), 1, fp);
	printf("%d elements read, sizeofLeNet5: %lu\n", read_amount, sizeof(LeNet5));
	fclose(fp);
	return 0;
}
...
void foo()
{
	...
	if (read_data(train_data, train_label, COUNT_TRAIN, FILE_TRAIN_IMAGE, FILE_TRAIN_LABEL))
	{
		...
		free(train_data);
		free(train_label);
		//system("pause"); // WINDOWS THING ONLY
	}
	...
	if (load(lenet, LENET_FILE))
		Initial(lenet); // start with random weights since no model.dat
	...
	if (DEBUG) printf("Saving trained model!\n");
	for (MyType *pos = lenet; pos < sizeof(LeNet5) + lenet; pos++) {
		printf("%f ", *pos);
	}
...
int main()
{
	...
	int correct = 0;
	int num_to_test = 2;
	for (int i = 0; i < num_to_test; i++) { // test some images
		image img;
