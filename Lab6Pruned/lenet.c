...
size_t PrintLayer(float *layer, size_t n) {
	size_t i = 0, zeros = 0;
	for(; i < n; i++) {
		if (layer[i] == 0.0f) zeros++;
		printf("%f ", layer[i]);
	}
	printf("\n");
	return zeros;
...
void PrintModel(LeNet5 *model) {
	#define HELPER(x) layer_size = sizeof(x);\
	num_elems = layer_size / sizeof(float);\
	printf(#x " weights:\n");\
	zeros += PrintLayer((float *)&x, num_elems)
